    ...
    !JR start-- string conservation equation!
    EV%s_ix=neq+1
    neq=neq+1
    maxeq=maxeq+1
    !JR end!
    ...
    real(dl) :: a 
    real(dl) :: adotoa
    real(dl) :: opacity_ignored, dopacity_ignored, ddopacity_ignored ! For IonizationFunctionsAtTime call
    real(dl) :: dvisibility_ignored, ddvisibility_ignored, lenswindow_ignored ! For IonizationFunctionsAtTime call
    real(dl) :: grhov_t, w_dark_energy_t ! For local adotoa calculation if needed
    integer  :: eigenmode
    real(dl) :: k_val_output_uetc, tau_val_output_uetc, ktau_val_output_uetc
    real(dl) :: u_S_raw_output_uetc, duS_dlogkt_raw_output_uetc
    real(dl) :: lambda_S_k_val_output_uetc, gamma_param_output_uetc
    real(dl) :: term_common_scaling_output_uetc
    real(dl) :: emtS_source_los_uetc, emtSdot_source_los_uetc
   
    real(dl) :: dot_numerator_S_output_uetc
    real(dl) :: adotoa_local_output 
    real(dl) :: grho_bkg, gpres_bkg 
    real(dl) :: visibility_at_tau, exptau_at_tau 
    real(dl) :: pi=const_pi
    ...
    !JR start!
    if (CP%Custom%active_mode_idx_uetc > 0) then
    ...
    !JR start!
    ! Variables for UETC source terms
    integer  :: eigenmode
    real(dl) :: k_val, tau_val, ktau
    real(dl) :: u_00, u_S
    real(dl) :: du00_dlogkt, duS_dlogkt
    real(dl) :: eigenvalue_k
    real(dl) :: gamma_loc
    real(dl) :: common_scaling_uetc
    real(dl) :: emt00, emtS,emt00dot, emtSdot, emtD, emtP
    real(dl) :: dot_numerator_00, dot_numerator_S
    real(dl) :: pi=const_pi

    real(dl) :: den_for_common_scaling, den_for_dot_terms
    
    real(dl) :: dum1, dum2


    logical, save :: has_done_interp_test = .false.
    real(dl), parameter :: k_test_f = 0.012648552168552958_dl   ! Same as K_TEST_VAL in Python
    real(dl), parameter :: tau_test_f = 0.046530709909382136_dl ! Same as TAU_TEST_VAL in Python
    integer, parameter :: mode_idx_test_f = 1                   ! Fortran 1-based (corresponds to Python 0-idx)
    integer, parameter :: type_idx_to_access_f = 1              ! 1 for u_00, 2 for u_S
                                                                ! (If testing derivatives, type index is still 1 or 2 but use derivative interpolator)
    character(len=20) :: component_name_f
    real(dl) :: f_interp_val
    ...
    !JR start!
    eigenmode = 0          ! Default to UETC off

    ! Initialize emt... terms to ensure they are defined even if UETC sources are off
    emt00 = 0.0_dl; emtS = 0.0_dl; emt00dot = 0.0_dl; emtSdot = 0.0_dl
    emtD = ay(EV%s_ix); ! emtD always reflects the current state of s_ix
    emtP = 0.0_dl       ! Will be updated if UETC is active and adotoa is valid

    if (allocated(CP%Custom)) then
        if (CP%Custom%active_mode_idx_uetc > 0) then 
            
            eigenmode = CP%Custom%active_mode_idx_uetc
            
            k_val = EV%k_buf  ! Use your variable name
            tau_val = tau     ! Use your variable name
            ktau = k_val * tau_val ! Use your variable name

            if (ktau > 1.0d-30 .and. tau_val > 1.0d-30) then ! Safety for ktau and tau positivity/non-zero

                u_00 = CP%Custom%ef_interp_00(eigenmode)%Value(k_val, ktau)
                u_S  = CP%Custom%ef_interp_S(eigenmode)%Value(k_val, ktau)
                du00_dlogkt = CP%Custom%ef_deriv_logkt_interp_00(eigenmode)%Value(k_val, ktau)
                duS_dlogkt  = CP%Custom%ef_deriv_logkt_interp_S(eigenmode)%Value(k_val, ktau)

                ! ---- Timing Lambda Interpolation ----
                eigenvalue_k = CP%Custom%lambda_interp_S(eigenmode)%Value(k_val)

                ! ---- End Timing Lambda Interpolation ----
                
                gamma_loc = 0.25_dl ! Use your variable 'gamma' if that's what you meant by renaming gamma_param_uetc
                                   ! Or use CP%Custom%weighting_uetc_stored if that is reliably 0.25

                den_for_common_scaling = (ktau**gamma_loc) * sqrt(tau_val)
                den_for_dot_terms = tau_val

                if (abs(den_for_common_scaling) > 1.0d-20) then
                    common_scaling_uetc = sqrt(abs(eigenvalue_k)) / den_for_common_scaling
                    if (eigenvalue_k < 0.0_dl) common_scaling_uetc = -common_scaling_uetc
                else
                    common_scaling_uetc = 0.0_dl
                endif

                emt00 = u_00 * common_scaling_uetc
                emtS  = u_S  * common_scaling_uetc

                dot_numerator_00 = du00_dlogkt - (gamma_loc + 0.5_dl) * u_00
                dot_numerator_S = duS_dlogkt - (gamma_loc + 0.5_dl) * u_S

                ! if (abs(den_for_dot_terms) > 1.0d-150) then
                !     emt00dot  = dot_numerator_00 * common_scaling_uetc / den_for_dot_terms
                !     emtSdot   = dot_numerator_S  * common_scaling_uetc / den_for_dot_terms
                ! else
                !     emt00dot  = 0.0_dl
                !     emtSdot   = 0.0_dl
                ! endif
                ...
                ! Ensure 'adotoa' here is the correctly calculated conformal Hubble rate for the current step
                if (abs(adotoa) > 1.0d-30) then 
                     emtP = (emtD - emt00dot)/adotoa - emt00
                else
                     emtP = -emt00 ! Avoid division by zero, simplified
                endif
            endif    
            
        endif ! End of main if (CP%Custom%active_mode_idx_uetc > 0 ...)
    endif ! End of if (allocated(CP%Custom))
    ! JR end!

    ! !JR 2
    ! if (allocated(CP%Custom) .and. .not. has_done_interp_test) then
    !     if (CP%Custom%active_mode_idx_uetc == 0 .and. & ! Only run if no actual UETC calculation is happening
    !         CP%Custom%uetc_interp_objects_are_set ) then ! And interpolators are ready
            
    !         print '(/,A,/)', "JR ----- FORTRAN INTERPOLATION TEST START -----"
    !         print '("JR TEST POINT: k=",ES18.10," tau=",ES18.10," mode_f=",I0," type_f=",I0)', &
    !               k_test_f, tau_test_f, mode_idx_test_f, type_idx_to_access_f

    !         ! Select which interpolator to test
    !         select case (type_idx_to_access_f)
    !         case (1) ! u_00
    !             component_name_f = "u_00"
    !             if (allocated(CP%Custom%ef_interp_00) .and. mode_idx_test_f <= size(CP%Custom%ef_interp_00)) then
    !                 f_interp_val = CP%Custom%ef_interp_00(mode_idx_test_f)%Value(k_test_f, tau_test_f)
    !                 print '("  Fortran interp ",A," : ",ES25.18E3)', trim(component_name_f), f_interp_val
    !             else
    !                 print *, "  Fortran: ef_interp_00 not ready for test."
    !             endif
    !         case (2) ! u_S
    !             component_name_f = "u_S"
    !             if (allocated(CP%Custom%ef_interp_S) .and. mode_idx_test_f <= size(CP%Custom%ef_interp_S)) then
    !                 f_interp_val = CP%Custom%ef_interp_S(mode_idx_test_f)%Value(k_test_f, tau_test_f)
    !                 print '("  Fortran interp ",A," : ",ES25.18E3)', trim(component_name_f), f_interp_val
    !             else
    !                 print *, "  Fortran: ef_interp_S not ready for test."
    !             endif
    !         ! Add cases for derivative interpolators if TYPE_IDX_TO_ACCESS_F implies them
    !         ! e.g., case(101) for du00_dlogkt, case(102) for duS_dlogkt (using different type numbers)
    !         case (101) ! du_00/dlogkt (Example encoding for derivatives)
    !             component_name_f = "du00/dlogkt"
    !              if (CP%Custom%uetc_deriv_interp_objects_are_set .and. &
    !                  allocated(CP%Custom%ef_deriv_logkt_interp_00) .and. mode_idx_test_f <= size(CP%Custom%ef_deriv_logkt_interp_00)) then
    !                 f_interp_val = CP%Custom%ef_deriv_logkt_interp_00(mode_idx_test_f)%Value(k_test_f, tau_test_f)
    !                 print '("  Fortran interp ",A," : ",ES25.18E3)', trim(component_name_f), f_interp_val
    !             else
    !                 print *, "  Fortran: ef_deriv_logkt_interp_00 not ready for test."
    !             endif
    !         case (102) ! du_S/dlogkt
    !             component_name_f = "duS/dlogkt"
    !              if (CP%Custom%uetc_deriv_interp_objects_are_set .and. &
    !                  allocated(CP%Custom%ef_deriv_logkt_interp_S) .and. mode_idx_test_f <= size(CP%Custom%ef_deriv_logkt_interp_S)) then
    !                 f_interp_val = CP%Custom%ef_deriv_logkt_interp_S(mode_idx_test_f)%Value(k_test_f, tau_test_f)
    !                 print '("  Fortran interp ",A," : ",ES25.18E3)', trim(component_name_f), f_interp_val
    !             else
    !                 print *, "  Fortran: ef_deriv_logkt_interp_S not ready for test."
    !             endif

    !         case default
    !             print *, "  Fortran ERROR: Invalid type_idx_to_access_f for interpolation test."
    !         end select

    !         print '("JR ----- FORTRAN INTERPOLATION TEST END -----",/) '
    !         has_done_interp_test = .true. ! Ensure it runs only once per CAMB execution
    !     endif
    ! endif
    ! !JR 2
    ...
    !JR start -- total density!
    if (eigenmode > 0 .and. k_val > 1.0d-20) then ! Use k_val, which is EV%k_buf
        dgrho = dgrho + 8.0d0*pi*emt00
        dgq = dgq - 8.0d0*pi*emtD/k_val
    endif
    ...
    real(dl) :: common_scaling_uetc, eigenvalue_k, gamma, pi=const_pi
    integer eigenmode
    !JR end!
    ...
    !     if (CP%Custom%active_mode_idx_uetc > 0 .and. &
    !         CP%Custom%uetc_interp_objects_are_set .and. &     ! Check eigenfunction interpolators
    !         CP%Custom%eigenvalue_interpolators_set .and. &    ! Check eigenvalue interpolators
    !         CP%Custom%uetc_deriv_interp_objects_are_set) then ! Check derivative interpolators
    !         gamma = 0.25
    !         eigenmode = CP%Custom%active_mode_idx_uetc
    ...
    !         ktau = k_val * tau_val
    !         ! eigenvectors
    !         u_V = CP%Custom%ef_interp_V(eigenmode)%Value(k_val, tau_val)
                
    !         eigenvalue_k = CP%Custom%lambda_interp_V(eigenmode)%Value(k_val)

    !         ! Calculate scaled source terms

    !         common_scaling_uetc = sqrt(abs(eigenvalue_k)) / &
    !                                            ( (ktau**gamma) * sqrt(tau_val) )
    !                 ! Add sign back if necessary, assuming lambda_S_k_val_uetc is mostly positive from eigh
    !         ! if (eigenvalue_k < 0.0_dl) common_scaling_uetc = -common_scaling_uetc

    !         ! emtV = u_V* common_scaling_uetc
    !         emtV=0

    !     else
    !         ! No active UETC mode or components not set
    !         emtV = 0.0_dl
    !     endif
    ! else
    !     ! CP%Custom not associated
    !     emtV = 0.0_dl
    ! endif
    ! ! !JR end!
    ...
    !         ktau = k_val * tau_val
    !         ! eigenvectors
    !         u_T = CP%Custom%ef_interp_T(eigenmode)%Value(k_val, ktau)
 
    !         !write(*,*) eigenmode, k_val, k_val * tau_val, u_T
    !         eigenvalue_k = CP%Custom%lambda_interp_T(eigenmode)%Value(k_val)
